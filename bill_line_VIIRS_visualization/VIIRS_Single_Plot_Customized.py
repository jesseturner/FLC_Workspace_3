...
data_dir = "bill_line_VIIRS_visualization/DNB_data_2024_06_20/" # directory where the data files reside for this case
  ...
  if key in geog_dict:
    ...
    for band in band:
      ...
      if prev_time is None or timediff1 > time2plot or prev_sat != sat or prev_band != band: # if new swath, plot old swath, then create new plot
        ...
        ax.coastlines(linewidth=1, color=border_color)
...
#          mask = (bt > 0) & (bt < 990) & (lon < -76)       
    ...
    # Plot the granule using pcolormesh. Modify cmap, vmin, vmax as needed
      ...
      if band in ["NCC"]:
        ...
        img = plot_rads.plot.pcolormesh(ax=ax, transform=ccrs.PlateCarree(), cmap=vis_cmap, vmin=0, vmax=1.5, add_colorbar=False)
        if latlon==1:
          #img2 = plot_rads2.plot.pcolormesh(ax=ax, transform=ccrs.PlateCarree(), cmap=vis_cmap, vmin=0, vmax=0.25, add_colorbar=False)
          img2 = plot_rads2.plot.pcolormesh(ax=ax, transform=ccrs.PlateCarree(), cmap=vis_cmap, vmin=0, vmax=1.5, add_colorbar=False)
        else:
          pass
        cbarl = "Albedo"
